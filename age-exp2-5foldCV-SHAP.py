...
feature_name = ['IndExpVar_D','Coverage_A','Coverage_C','MeanGFP_A','MeanGFP_B','MeanGFP_C','MeanGFP_D','MeanGFP_E',
                'OrgTM_A->B','OrgTM_A->D','OrgTM_A->E','OrgTM_B->C','OrgTM_C->B','OrgTM_D->A','OrgTM_E->A','OrgTM_E->D',
                'DeltaTM_A->B','DeltaTM_E->C','DeltaTM_E->D','DurationStdDev_A','GFPStdDev_A','GFPStdDev_B','GFPStdDev_C',
                'GFPStdDev_D','GFPStdDev_E','DefaultMode_Cosine similarity','DefaultMode_Euclidean distance','SensoriMotor_Pearson correlation',
                'SensoriMotor_Cosine similarity','SensoriMotor_Euclidean distance','Visual_Pearson correlation','Visual_Cosine similarity',
                'Visual_Euclidean distance','Salience_Pearson correlation','Salience_Cosine similarity','Salience_Euclidean distance',
                'DorsalAttention_Pearson correlation','DorsalAttention_Cosine similarity','DorsalAttention_Euclidean distance','FrontoParietal_Pearson correlation',
                'FrontoParietal_Cosine similarity','FrontoParietal_Euclidean distance','Language_Pearson correlation','Language_Cosine similarity',
                'Language_Euclidean distance','Cerebellar_Pearson correlation','Cerebellar_Cosine similarity','Cerebellar_Euclidean distance',
                'Whole-brain_PearsonCorrelation','Whole-brain_CosineSimilarity','Whole-brain_EuclideanDistance'
]
...
file_path = 'J:\Project\EEG_fMRI_Statistics\SHAP-CNN\EEG+fMRI输入数据new(174)-实验四-51.xlsx'  # 替换为你的Excel文件路径
...
for age_i in [1,2,3,4]:        #1-5    年龄
    if age_i == 1:
        ...
        K = 0.89
        model_A = 'GradientBoosting'
        model_B = 'xgboost'
    else:
        K = 0.7
        model_A = 'RandomForest'
        model_B = 'DecisionTreeClassifier'
    folder_path_age = f"J:\\Project\\EEG_fMRI_Statistics\\SHAP-CNN\\new联合模型实验记录(Mann-Whitney)\\SHAP\\五折\\实验四\\{model_A}+{model_B}\\按年龄筛选数据-交叉验证-trick2-new-51\\年龄分类" + str(age_i)
    ...
    for illness_i in [1]:        #0和5   疾病
        max_KFold_Accuracy1 = 0
        folder_path_illness = folder_path_age+'/疾病分类'+ str(illness_i)
        ...
        filtered_df = df[(df.iloc[:, 5] == age_i) & (df.iloc[:, 2].isin([0, illness_i]))]  # 第一个是年龄，第二个是疾病类型
        ...
        for random_i in range(10):
            ...
            # for random_i in range(30):
            #     X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=random_i)
                ...
                if max_KFold_Accuracy1 < accuracy:
                    max_KFold_Accuracy1 = accuracy
                    #######################################################################################################SHAP
                    ...
                    if model_B == 'xgboost':
                        explainer_B = shap.TreeExplainer(ModelB)
                        shap_values_B = explainer_B(X_test)
                        print(shap_values_A.shape,
                              shap_values_B.shape)  # B会生成一个(35, 38, 2)的数组，我们只要[:, :, 1]，为1的shap
                        #
                        ...
                        combined_shap = K * shap_values_A[:, :, 1] + (2 - K) * shap_values_B
                    else:
                    ...
                    shap.summary_plot(combined_shap, X_test, max_display=51,
                                      feature_names=feature_name)  # 注意这里X_test[:10]仅用于摘要图的x轴标签，实际数据是shap_values对应的
                    plt.savefig(
                        folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_融合健康点图.png")
                    plt.close()
                    print('保存至：')
                    print(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_融合健康点图.png")
                    shap.summary_plot(combined_shap, X_test,
                                      plot_type="bar", max_display=51, feature_names=feature_name)
                    plt.savefig(
                        folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_融合健康条形图.png")
                    plt.close()
        ...
        with open(txt_illness, "w", encoding="utf-8") as file:
            for line in lines_to_write:
                file.write(line + "\n")
#########################################在最佳里再训练一遍
#         feature_name = ['IndExpVar_D','MeanDuration_C','Coverage_C','MeanGFP_A','MeanGFP_B','MeanGFP_C','MeanGFP_D',
#                         'MeanGFP_E','OrgTM_B->C','OrgTM_C->B','OrgTM_E->A','DeltaTM_A->B','DeltaTM_B->C','DeltaTM_E->A'
#             ,'DeltaTM_E->D','GFPStdDev_A','GFPStdDev_B','GFPStdDev_C','GFPStdDev_D','GFPStdDev_E','DefaultMode_Euclidean distance'
#             ,'SensoriMotor_Pearson correlation','SensoriMotor_Euclidean distance','Visual_Pearson correlation'
#             ,'Visual_Cosine similarity','Visual_Euclidean distance','Salience_Pearson correlation','Salience_Cosine similarity'
#             ,'Salience_Euclidean distance','DorsalAttention_Pearson correlation','DorsalAttention_Euclidean distance'
#             ,'FrontoParietal_Pearson correlation','FrontoParietal_Cosine similarity','FrontoParietal_Euclidean distance'
#             ,'Language_Pearson correlation','Language_Cosine similarity','Language_Euclidean distance','Cerebellar_Pearson correlation'
#             ,'Cerebellar_Euclidean distance','whole-brain_Pearson correlation','whole-brain_Cosine similarity','whole-brain_Euclidean distance'
# ]
#         X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=max_random)
#         dtrain = xgb.DMatrix(X_train, label=y_train)
#         dtest = xgb.DMatrix(X_test, label=y_test)
#         num_round = 100  # 迭代次数
#         bst = xgb.train(params, dtrain, num_round)
...
#         print("shap_values_xgb " ,shap_values_xgb.shape)
#         print("type(shap_values_xgb) " ,type(shap_values_xgb))
#         shap_xgb_std = standardize_shap_values(shap_values_xgb)
#         # shap_values_xgb = (shap_values_xgb - np.mean(shap_values_xgb)) / np.std(shap_values_xgb)
#         shap.summary_plot(shap_values_xgb , X_test, max_display=42, feature_names=feature_name)  # 注意这里X_test[:10]仅用于摘要图的x轴标签，实际数据是shap_values对应的
#         plt.savefig(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_xgboost点状图.png")
#         plt.close()
#         shap.summary_plot(shap_values_xgb , X_test,
#                           plot_type="bar", max_display=42, feature_names=feature_name)
#         plt.savefig(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_xgboost条形图2.png")
#         plt.close()
#
...
#         # model = svm.SVC(kernel='linear')  # 你可以尝试不同的核函数，如'rbf', 'poly'等
#         model.fit(X_train, y_train)
#         y_pred, y_pred_Probability = model.predict(X_test)
#         y_pred_Probability = y_pred_Probability[:, 1]  # 形状 (43,)
#         #####
#         avg_arr = (y_pred_Probability + y_pred) / 2
#
#         y_pred = (avg_arr >= 0.5).astype(int)
...
#
#         shap_rf_std = standardize_shap_values(shap_values_rf)
#         print("shap_rf_std " ,shap_rf_std.shape)
#         print("type(shap_rf_std) " ,type(shap_rf_std))
#         sample_index = 0
#         # shap.force_plot(explainer.expected_value[1], shap_values[1][sample_index, :], X_test[sample_index, :])
#         # plt.savefig(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_力图.png")
#         # 使用SHAP库进行可视化
#         # shap_values是一个列表，其中每个元素对应一个类别的SHAP值
#         # 对于二分类问题，通常有两个元素（正类和负类）
#         # 我们通常对正类的SHAP值感兴趣（或者根据你的具体问题选择）
#         shap.summary_plot(shap_rf_std, X_test, max_display=42, feature_names=feature_name)  # 注意这里X_test[:10]仅用于摘要图的x轴标签，实际数据是shap_values对应的
#         plt.savefig(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_RandomForest健康点图.png")
#         plt.close()
#
#         shap.summary_plot(shap_rf_std, X_test,
#                           plot_type="bar", max_display=42, feature_names=feature_name)
#         plt.savefig(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_RandomForest健康条形图.png")
#         plt.close()
#         k = k_Average
#         combined_shap = k * shap_xgb_std + (1 - k) * shap_rf_std
#         shap.summary_plot(combined_shap, X_test, max_display=42, feature_names=feature_name)  # 注意这里X_test[:10]仅用于摘要图的x轴标签，实际数据是shap_values对应的
...
#         plt.close()
#
#         # shap.summary_plot(shap_values_rf[1], X_test, max_display=38)  # 注意这里X_test[:10]仅用于摘要图的x轴标签，实际数据是shap_values对应的
#         #
#         # plt.savefig(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_RandomForest疾病点图.png")
#         # plt.close()
#         #
#         # shap.summary_plot(shap_values_rf[1], X_test,
#         #                   plot_type="bar", max_display=38)
#         # plt.savefig(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_RandomForest疾病条形图.png")
#         # plt.close()
