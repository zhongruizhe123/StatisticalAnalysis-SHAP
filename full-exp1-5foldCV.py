...
for model_A_num in range(len(model_list)-1):
    for model_B_num in range(model_A_num+1, len(model_list)):
        ...
        folder_path_age = f"J:\\Project\\EEG_fMRI_Statistics\\SHAP-CNN\\new联合模型实验记录(Mann-Whitney)\\五折\\实验一\\{model_A}+{model_B}\\按年龄筛选数据-交叉验证-trick2-new-51\\"
        ...
        for illness_i in [1]:        #0和5   疾病
            ...
            with open(txt_illness, "w", encoding="utf-8") as file:
                for line in lines_to_write:
                    file.write(line + "\n")
#########################################在最佳里再训练一遍
    #         feature_name = ['IndExpVar_D', 'MeanDuration_C', 'Coverage_C', 'MeanGFP_A', 'MeanGFP_B', 'MeanGFP_C',
    #                         'MeanGFP_D',
    #                         'MeanGFP_E', 'OrgTM_B->C', 'OrgTM_C->B', 'OrgTM_E->A', 'DeltaTM_A->B', 'DeltaTM_B->C',
    #                         'DeltaTM_E->A'
    #             , 'DeltaTM_E->D', 'GFPStdDev_A', 'GFPStdDev_B', 'GFPStdDev_C', 'GFPStdDev_D', 'GFPStdDev_E',
    #                         'DefaultMode_Euclidean distance'
    #             , 'SensoriMotor_Pearson correlation', 'SensoriMotor_Euclidean distance', 'Visual_Pearson correlation'
    #             , 'Visual_Cosine similarity', 'Visual_Euclidean distance', 'Salience_Pearson correlation',
    #                         'Salience_Cosine similarity'
    #             , 'Salience_Euclidean distance', 'DorsalAttention_Pearson correlation',
    #                         'DorsalAttention_Euclidean distance'
    #             , 'FrontoParietal_Pearson correlation', 'FrontoParietal_Cosine similarity',
    #                         'FrontoParietal_Euclidean distance'
    #             , 'Language_Pearson correlation', 'Language_Cosine similarity', 'Language_Euclidean distance',
    #                         'Cerebellar_Pearson correlation'
    #             , 'Cerebellar_Euclidean distance', 'whole-brain_Pearson correlation', 'whole-brain_Cosine similarity',
    #                         'whole-brain_Euclidean distance'
    #                         ]
    ...
    # plt.savefig(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_点状图.png")
    # plt.close()
    # shap.summary_plot(shap_values, X_test,
    #                   plot_type="bar", max_display=32, feature_names=feature_name)
    # plt.savefig(folder_path_illness + '/' + str(age_i) + '_' + str(illness_i) + "_条形图2.png")
    # plt.close()
#########################################在最佳里再训练一遍
